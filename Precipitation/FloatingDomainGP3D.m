function [mu, var, preprocessingtime, inferencetime, max_set_length, I]=FloatingDomainGP3D(x,y,x_s,Omega,N_u,sigma_SE,l_SE,dist,r,sigma_y)
...
M_max1=ceil(2*max(r(1),dist(1))./l_u1);
M_max2=ceil(2*max(r(2),dist(2))./l_u2);
M_max3=ceil(2*max(r(3),dist(3))./l_u3);
...
for t=1:N_s
    ...
    [set1, set2, set3, xu_set]=find_set3D(x_s(:,t),Omega,l_u1,l_u2,l_u3,N_u,x1_u,x2_u,x3_u,dist);
...
inferencetime=toc(predtime)./N_s;
